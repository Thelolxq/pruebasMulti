  ...
  const handleClose =()=>{
   ...
   const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch("http://192.168.0.114:8081/patients/2", formData);
    ...
    <div className="w-11/12  rounded-xl shadow-md bg-white flex gap-1 h-5/6 ">
      ...
      {selectedPatient && (
        <div className=" flex w-full text-sm text-white  flex-col border-b gap-1 h-full p-5">
          ...
          <button
          onClick={handleOpen}
          className="cursor-pointer hover:scale-105 duration-300 w-32 h-10 rounded-xl bg-yellow-600 shadow-xl">editar</button>
